...
class SisterV1(BaseStrategy):
    ...
    def start(self, market_data) -> None:
        ...
        self.atr_entry = ta.atr(
            high=self.high,
            low=self.low,
            close=self.close,
            length=self.params['length_entry']
        )
        self.sma_entry = ta.sma(
            source=self.close,
            length=self.params['length_entry']
        )
        self.sma_long_entry = (
        ...
        self.atr_exit = ta.atr(
            high=self.high,
            low=self.low,
            close=self.close,
            length=self.params['length_exit']
        )
        self.sma_exit = ta.sma(
            source=self.close,
            length=self.params['length_exit']
        )
        self.sma_long_exit = (
        ...
        self.sma_small_trend = ta.sma(
            source=self.close,
            length=self.params['length_small_trend']
        )
        self.sma_medium_trend = ta.sma(
            source=self.close,
            length=self.params['length_medium_trend']
        )

        self.cond_exit_long = ta.crossover(
            source1=self.close,
            source2=self.sma_long_exit
        )
        self.cond_exit_short = ta.crossover(
            source1=self.close,
