...
class NuggetV4():
    ...
    opt_parameters = {
        ...
        'take_volume_2': [
            math.random_trading_volumes(10, 5, 5)
            for i in range(10000)
        ],
    ...
    def trade(self):
        ...
        if self.alert_long_1:
            ...
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_1[0]),
                price=self.take_price[0][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_1[1]),
                price=self.take_price[1][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_1[2]),
                price=self.take_price[2][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_1[3]),
        ...
        if self.alert_long_2:
            ...
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[0]),
                price=self.take_price[0][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[1]),
                price=self.take_price[1][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[2]),
                price=self.take_price[2][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[3]),
                price=self.take_price[3][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[4]),
                price=self.take_price[4][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[5]),
                price=self.take_price[5][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[6]),
                price=self.take_price[6][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[7]),
                price=self.take_price[7][-1],
                hedge='false'
            )
            self.http_client.futures_limit_take_sell(
                symbol=self.http_client.symbol,
                size='{}%'.format(self.take_volume_2[8]),
