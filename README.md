...
Скачайте и установите [Python](https://www.python.org/downloads/) с официального сайта. При установке обязательно активируйте опцию **Add Python to PATH**.
...
**a) Получение токена бота:**

1. Напишите [@BotFather](https://t.me/BotFather) команду `/start`.
2. Создайте нового бота через команду `/newbot`.
3. Сохраните полученный API-токен.

**b) Получение Telegram chat ID:**
...
2. Перенесите файлы с оптимальными параметрами из папки `optimization` в папку `backtesting`. В начало каждого файла можно добавить строку с временным периодом (например, `Period: 2017-01-01 - 2019-01-01`).
